...
def list_files(service, folder_id, extension=None):
...
def download_file(service, file_id, destination=None):
...
def upload_file(service, name, path, destination_folder=None):
...
def move_file(service, file_id, new_folder, old_folder=None):
    return service.files().update(fileId=file_id,
                                  addParents=new_folder,
                                  removeParents=old_folder).execute()

def read_local_file(file_path, encoding='latin-1'):
    contents = ''
